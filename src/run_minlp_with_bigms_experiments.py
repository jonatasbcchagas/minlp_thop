...
def launcher(tsp_base, number_of_items_per_city, knapsack_type, knapsack_size, maximum_travel_time):
    ...
    outputfile = "../solutions/minlp_bigms/%s-thop/%s_%02d_%s_%02d_%02d.thop.sol" % (tsp_base, tsp_base, number_of_items_per_city, knapsack_type, knapsack_size, maximum_travel_time)
    if os.path.isfile(outputfile): return
    os.system("python main.py -i %s -s %s -t 3600 -m > %s" % (inputfile, outputfile, outputfile+".log"))
